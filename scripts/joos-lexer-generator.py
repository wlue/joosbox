...
for name, kind, default in tokens:
    if kind == "Fixed":
        if default:
            ...
            raise ValueError("Fixed token type must have a default value.")
    elif kind == "Variable":
...
for name, kind, default in tokens:
    if kind == "Fixed":
        if default:
            if is_keyword(default):
                print case_class_template % (name, "KeywordToken")
            else:
                print case_class_template % (name, "FixedToken")
        else:
            raise ValueError("Fixed token type must have a default value.")
    elif kind == "Variable":
        print case_class_template % (name, "VariableToken")
